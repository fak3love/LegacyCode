...
namespace MonoScript.Script
{
    public class Using
    {
        string _path;
        string _asName;
        public string Path { get => _path; set { if (!string.IsNullOrEmpty(value) && !MonoObject.IsCorrectPath(value)) MLog.AppErrors.Add(new AppMessage("Invalid Path 'Using'", value)); _path = value; } }
        public string AsName { get => _asName; set { if (!string.IsNullOrEmpty(value) && !MonoObject.IsCorrectName(value)) MLog.AppErrors.Add(new AppMessage("Invalid AsName 'Using'", value)); _asName = value; } }
